...
names(var_train_soil) <- stringr::str_split(names(var_train_soil), "_", simplify = TRUE)[,1]
names(var_train_soil)
...
for(i in sort(unique(occ$species))){
    ...
    # directories ----
    sp_dir_sp <- paste0("02_output/01_sdm_rare/", sp_name, "/")
    sp_dir_pre_model <- paste0("02_output/01_sdm_rare/", sp_name, "/01_pre_modeling/")
    ...
    occ_i_filt_psa_part_occ <- flexsdm::part_random(
        data = occ_i_filt_psa,
        pr_ab = "pr_ab",
        method = c(method = "rep_kfold", folds = part_n, replicates = 5))
    
    occ_i_filt_psa_part_info <- tibble::tibble(method = "rep_kfold", folds = part_n, replicates = 5)
    ...
    rm(var_train_ca, var_train_ca_sel)
