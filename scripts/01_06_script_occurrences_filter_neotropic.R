#' ----
#' title: obj2 - occurrences - neotropic filter
...
# limits
lim_neo <- sf::st_read("01_data/02_variables/00_limits/neotropic/neotropic_dissolved_fill_holes.shp")
lim_neo
plot(lim_neo$geometry)
...
## sf ----
occ_atlantic_epiphytes_sf <- occ_atlantic_epiphytes %>% 
  dplyr::mutate(lon = decimalLongitude,
                lat = decimalLatitude) %>% 
  sf::st_as_sf(coords = c("lon", "lat"), crs = 4326)
occ_atlantic_epiphytes_sf

## filter ----
occ_atlantic_epiphytes_neo <- occ_atlantic_epiphytes_sf %>% 
  dplyr::mutate(neotropic = as.logical(sf::st_intersects(occ_atlantic_epiphytes_sf, lim_neo, sparse = FALSE)))
occ_atlantic_epiphytes_neo

## report ----
occ_atlantic_epiphytes_neo_report <- 
occ_atlantic_epiphytes_neo_report

## filter ----
occ_atlantic_epiphytes_neo_output <- occ_atlantic_epiphytes_neo %>% 
  dplyr::filter(neotropic == 1)
