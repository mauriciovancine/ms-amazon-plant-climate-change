...
# current ----
for(i in sort(unique(occ$species))[4:7]){
    ...
    sp_dir_pos_model <- paste0("02_output/01_sdm_rare/", sp_name, "/03_pos_modeling/")
    ...
    # model performance ----
    model_performance <- readr::read_csv(paste0(sp_dir_model, "03_04_mod_performance_", sp_name, ".csv"))
    ...
    if(length(good_models) > 0){
        ...
        ## ensemble ----
        ens_prd <- dir(sp_dir_pos_model, pattern = "_current.tif", full.names = TRUE) %>%
            stringr::str_subset("model_good_models_") %>% 
            terra::rast() %>% 
            terra::weighted.mean(model_performance$SORENSEN_mean)
        
        ens_prd_thr <- terra::ifel(ens_prd >= mean(model_performance$thr_value), 
                                   ens_prd, 0)
...
for(i in sort(unique(occ$species))){
    ...
    for(f in fut){
        ...
        ## select variables ----
        var_names <- readr::read_csv(paste0(sp_dir_model, "03_03_mod_importance_", sp_name, ".csv")) %>% 
            dplyr::distinct(predictors) %>% 
            dplyr::pull()
        ...
        if(length(good_models) > 0){
            ...
            ## predict ensemble ----
            ens_prd <- dir(sp_dir_pos_model, pattern = paste0("_future_", f, ".tif"), full.names = TRUE) %>%
                stringr::str_subset("model_good_models_") %>% 
                terra::rast() %>% 
                terra::weighted.mean(model_performance$SORENSEN_mean)
